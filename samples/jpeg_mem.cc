...
uint8* UncompressLow(const void* srcdata, FewerArgsForCompiler* argball) {
  ...
  if (cinfo.output_width <= 0 || cinfo.output_height <= 0) {
    std::cout << "Invalid image size: " << cinfo.output_width << " x "
               << cinfo.output_height;
    jpeg_destroy_decompress(&cinfo);
    return nullptr;
  }
  ...
  if (flags.crop) {
    ...
    if (!IsCropWindowValid(flags, cinfo.output_width, cinfo.output_height)) {
      std::cout << "Invalid crop window: x=" << flags.crop_x
    ...
    if(skipped_scanlines != flags.crop_y)
  ...
  while (cinfo.output_scanline < max_scanlines_to_read) {
    ...
    // Handle error cases
    if (num_lines_read == 0) {
      std::cout << "Premature end of JPEG data. Stopped at line "
